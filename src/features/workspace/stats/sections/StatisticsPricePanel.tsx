...
export function StatisticsPricePanel({
  ...
  const priceCategoryLabel = priceIntelligence.categoryLabel ?? priceIntelligence.contextLabel ?? '—';
  ...
  const priceTicks = hasRangeValues
    ? [
        ...
        { key: 'optimal-min', label: priceIntelligence.optimalMinLabel ?? endpointMinLabel, percent: optimalLeftPercent },
        { key: 'avg', label: priceIntelligence.marketAverageLabel ?? '—', percent: averageMarkerPercent },
        { key: 'optimal-max', label: priceIntelligence.optimalMaxLabel ?? endpointMaxLabel, percent: optimalRightPercent },
        { key: 'max', label: endpointMaxLabel, percent: 100 },
      ].filter((tick, index, collection) => {
        return collection.findIndex((candidate) => candidate.label === tick.label && candidate.percent === tick.percent) === index;
      })
    : [];
        ...
        <article className="workspace-statistics-price__radar-card">
          <header className="workspace-statistics-price__radar-head">
            <div className="workspace-statistics-price__market-line">
              <p className="request-category workspace-statistics-price__context">{priceCategoryLabel}</p>
              {priceCityLabel ? (
                <LocationMeta
          ...
          {hasRangeValues ? (
            <div className="workspace-statistics-price__radar-body" role="group" aria-label={copy.priceRadarLabel}>
              ...
              <div className="workspace-statistics-price__average">
                ...
                <strong className="proof-price">{priceIntelligence.marketAverageLabel ?? '—'}</strong>
              </div>
            </div>
          ) : null}
        </article>
