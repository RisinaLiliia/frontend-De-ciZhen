...
import { WorkspaceSectionAside, buildLinkedWorkspaceRailModel } from '@/features/workspace/shared';
...
export function WorkspaceReviewsAside({ t, locale, hideBelowTablet = true }: Props) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      withStatusFallback(() => getWorkspaceReviews({ range: reviewRange, sort }), null, [400, 404]),
    retry: false,
    staleTime: 60_000,
  });
  ...
  return (
    <WorkspaceSectionAside model={model} isLoading={isLoading} hideBelowTablet={hideBelowTablet}>
      {data?.composer.enabled ? (
        <WorkspaceReviewsComposer t={t} requiresAuthorName={data.composer.requiresAuthorName} />
      ) : null}
