...
export function hasOwnerRequestEditCapability(
  card: Pick<WorkspaceMyRequestCardDto, 'role' | 'status' | 'canEdit' | 'capabilities'>,
) {
...
export function hasOwnerRequestManagementCapability(
  card: Pick<
  ...
  if (
    card.role === 'customer' &&
    (card.canEdit || card.canDelete || card.canDuplicate || card.canRestore)
  ) {
    return true;
  }

  return card.status.actions.some(
    (action) =>
...
export function resolveOwnerMenuActions(params: { card: WorkspaceMyRequestCardDto }) {
  const { card } = params;
  const sourceActions =
    card.menuActions && card.menuActions.length > 0 ? card.menuActions : card.status.actions;
