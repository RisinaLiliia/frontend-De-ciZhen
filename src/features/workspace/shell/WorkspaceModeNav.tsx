...
export function WorkspaceModeNav({ items, t, variant = 'cards' }: WorkspaceModeNavProps) {
  ...
  return (
    <nav className={rootClassName} aria-label={t(I18N_KEYS.workspace.modeNavAriaLabel)}>
      {items.map((item) => (
        <Link
          key={item.key}
          href={item.href}
