...
import type { WorkspaceContextExtraFilter } from '@/features/workspace/context/workspaceContext.types';
...
export function WorkspaceContextInlineControls({
  inlineControl,
  extraFilters,
}: Props) {
  ...
  const controlClusterClassName = inlineControl && !extraFilters?.length
  ...
  return (
    <div className={controlClusterClassName}>
      {inlineControl ? (
        <div className="workspace-context-controls__inline-control">
          {inlineControl}
        </div>
      ) : null}
      ...
      {extraFilters?.map((filter) => (
        <div
          key={filter.key}
          className={filter.display === 'chips' ? 'workspace-context-controls__chip-filter' : undefined}
        >
