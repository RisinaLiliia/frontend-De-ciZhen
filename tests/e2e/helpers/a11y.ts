...
export async function expectNoAxeViolations(page: Page, options?: AxeAuditOptions) {
  ...
  const violations = await page.evaluate(
    async ({ excludeSelectorsArg }) => {
      const axeRef = (
        window as typeof window & {
          axe?: { run: (...args: unknown[]) => Promise<{ violations: AxeViolation[] }> };
        }
      ).axe;
      if (!axeRef) return [];
  ...
  expect(blocking, blocking.map((violation) => formatViolation(violation)).join('\n\n')).toEqual(
    [],
  );
}
